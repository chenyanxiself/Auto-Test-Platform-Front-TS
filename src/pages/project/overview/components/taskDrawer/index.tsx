...
import { Drawer, Form, Input, message, Spin, Row, Col, Tooltip, Avatar, Divider } from 'antd';
import { LoadingOutlined } from '@ant-design/icons';
import { connect } from 'umi';
import { Dispatch } from '@@/plugin-dva/connect';
import { getTaskDetail, updateTask, uploadTaskImgApi } from '@/pages/project/overview/service';
...
const TaskDrawer: React.FC<TaskDrawerProps> = (props) => {
    ...
    const cname = props.currentTask.creator ? props.currentTask.creator.cname : '';
    useEffect(() => {
    ...
    const saveTitleHandle = async (e) => {
      ...
      const res = await updateTask(props.projectId, props.currentTask.id, v, 'title');
      if (res.status !== 1) {
    ...
    const saveDescriptionHandle = async (e) => {
      const v = e.target.value;
      if (v !== props.currentTask.description) {
        const res = await updateTask(props.projectId, props.currentTask.id, v, 'description');
        if (res.status !== 1) {
          return message.warning(res.error);
        }
        message.success('修改成功');
    ...
    const savePriorityHandle = async (key) => {
      if (key !== form.getFieldValue('taskPriority')) {
        const res = await updateTask(props.projectId, props.currentTask.id, key, 'priority');
        if (res.status !== 1) {
          return message.warning(res.error);
        }
        message.success('修改成功');
    ...
    const saveStatusHandle = async (key) => {
      if (key !== form.getFieldValue('taskStatus')) {
        const res = await updateTask(props.projectId, props.currentTask.id, key, 'status');
        if (res.status !== 1) {
          return message.warning(res.error);
        }
        message.success('修改成功');
    ...
    const saveFollowerHandle = async (v) => {
      ...
      const ori_id_list = JSON.stringify(form.getFieldValue('taskFollower').map(item => item.id).sort());
      if (id_list !== ori_id_list) {
        const res = await updateTask(props.projectId, props.currentTask.id, id_list, 'follower');
        if (res.status !== 1) {
          return message.warning(res.error);
        }
        message.success('修改成功');
              ...
              <Row>
                ...
                <Col span={24}>
                  <div className={styles.creatorInfo}>
                    ...
                    <div>{cname} 创建于 {props.currentTask.createTime ? new Date(+new Date(props.currentTask.createTime) + 8 * 3600 * 1000).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '') : null}</div>
                  </div>
                </Col>
                <Divider />
                <Col span={24}>
